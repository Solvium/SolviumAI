  ...
  const playGame = async () => {
    ...
    try {
      ...
      const background = new headbreaker.Canvas("canvas", {
        ...
        pieceSize: (userDetails?.level ?? 1) + (userDetails?.difficulty ?? 1) == 2 ? 80 : Math.round(pieceSize - 20),
        proximity: 18,
        borderFill: 8,
        strokeWidth: 1,
        lineSoftness: 0.12,
      ...
      background.autogenerate({
        horizontalPiecesCount: (userDetails?.level ?? 1) + (userDetails?.difficulty ?? 1),
        verticalPiecesCount: (userDetails?.level ?? 1) + (userDetails?.difficulty ?? 1),
      })
      ...
      background.onValid(() => {
        ...
        const points = timeDiff < 120 ? 100 : timeDiff > 120 && timeDiff < 240 ? 75 : 50

        setTimeout(() => {
          setPoints(points * (userDetails?.level ?? 1) * (multiplier > 0 ? multiplier : 1))
          setSolved(true)
          setSaving(true)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0A0146] via-[#1a0a3e] to-[#2d1b69] text-white relative overflow-hidden">
      ...
      <div
        ...
        style={{ animationDelay: "0.5s", boxShadow: "0 0 8px rgba(192, 132, 252, 0.8)" }}
      ></div>
      <div
        className="absolute bottom-[35%] right-8 w-3 h-3 bg-pink-500 rounded-full animate-pulse"
        style={{ animationDelay: "1s", boxShadow: "0 0 10px rgba(236, 72, 153, 0.8)" }}
      ></div>
      <div
        className="absolute top-[60%] right-12 w-2 h-2 bg-purple-300 rounded-full animate-pulse"
        style={{ animationDelay: "1.5s", boxShadow: "0 0 8px rgba(216, 180, 254, 0.8)" }}
      ></div>
      ...
      <div className="flex items-center justify-between px-4 pt-6 pb-4 max-w-md mx-auto">
        ...
        <h1 className="text-3xl font-bold text-white tracking-[0.3em]" style={{ fontFamily: "monospace" }}>
          PUZZLE
        </h1>
        ...
        <div className="max-w-md mx-auto px-4 mt-6 mb-8">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-start gap-2 flex-1">
              <span className="text-pink-500 text-lg mt-0.5" style={{ textShadow: "0 0 8px rgba(236, 72, 153, 0.6)" }}>
                ✦
              </span>
            ...
            <div className="w-32 h-24 rounded-lg overflow-hidden border-2 border-cyan-400/40 flex-shrink-0 shadow-lg">
              <img src={curImg.src || "/placeholder.svg"} alt="Puzzle preview" className="w-full h-full object-cover" />
            </div>
          </div>
        </div>
      )}
        ...
        <div className="max-w-md mx-auto px-4 mb-6">
          ...
          <div className="relative">
            ...
            <div
              ...
              style={{ boxShadow: "0 0 40px rgba(34, 211, 238, 0.3), inset 0 0 20px rgba(34, 211, 238, 0.1)" }}
            >
      ...
      {solved && (
        <div className="max-w-md mx-auto px-4 text-center space-y-4">
          <div className="text-2xl font-bold text-green-400">Congratulations! Puzzle Solved!</div>
