...
export default function MobileWordSearchGame(): ReactElement {
  ...
  const toggleSound = () => {
    ...
    localStorage.setItem("wordHunt_soundEnabled", JSON.stringify(newSoundEnabled))
  ...
  const swipeDifficulty = (direction: "next" | "prev") => {
    ...
    let newIndex
    if (direction === "next") {
      newIndex = currentIndex === difficulties.length - 1 ? 0 : currentIndex + 1
    } else {
      newIndex = currentIndex === 0 ? difficulties.length - 1 : currentIndex - 1
    }
    ...
    // Show swipe hint
    setShowSwipeHint(`Switched to ${newDifficulty.charAt(0).toUpperCase() + newDifficulty.slice(1)}`)
  ...
  // Calculate timer
  const calculateTimer = useCallback((level: number, difficulty: string): number => {
    const baseTimes = { easy: 90, medium: 120, hard: 180 }
    const scalingFactor = 1 + (level - 1) * 0.1
    return Math.round(baseTimes[difficulty as keyof typeof baseTimes] * scalingFactor)
  }, [])
  ...
  const initializePuzzle = useCallback(() => {
    ...
    const puzzle = generateWordSearch(themeData.words, gridSize, gameStats.difficulty)
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    e.preventDefault()
    if (!isSelecting || !selectionStart || !currentPuzzle || !gridRef.current) return
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    e.preventDefault()
    if (!isSelecting || !currentPuzzle || !selectionStart || !selectionEnd) return

    setIsSelecting(false)

    const selectedText = currentSelection.map((cell) => currentPuzzle.grid[cell.row]?.[cell.col]?.letter || "").join("")
    ...
    for (const word of currentPuzzle.words) {
      if (!word.found && (word.word === selectedText || word.word === reversedText)) {
        foundWord = word
        break
      }
    }
    ...
    if (foundWord) {
      const newPuzzle = { ...currentPuzzle }
      const wordIndex = newPuzzle.words.findIndex((w) => w.id === foundWord!.id)
  ...
  const handlePuzzleCompletion = useCallback(() => {
    ...
    setGameStats((prev) => ({
      ...
      completedPuzzles: new Set([...prev.completedPuzzles, currentPuzzle?.id || ""]),
    }))
  ...
  const useHint = useCallback(
    (type: "reveal" | "highlight") => {
      ...
      if (gameStats.hintsRemaining > 0) {
        setGameStats((prev) => ({ ...prev, hintsRemaining: prev.hintsRemaining - 1, hintsUsed: prev.hintsUsed + 1 }))
      } else {
        setGameStats((prev) => ({
          ...prev,
      ...
      if (type === "reveal") {
        const wordIndex = newPuzzle.words.findIndex((w) => w.id === randomWord.id)
        newPuzzle.words[wordIndex].found = true
        ...
        setTimeout(() => {
          ...
          cells.forEach((cell) => {
            if (tempPuzzle.grid[cell.row] && tempPuzzle.grid[cell.row][cell.col]) {
