...
export class SecureWalletStorage {
  ...
  async storeWalletData(
    ...
    try {
      ...
      console.log(
        `[WalletStorage] Stored wallet data for user ${telegramUserId}`
      );
    } catch (error) {
      console.error("[WalletStorage] Failed to store wallet data:", error);
      throw error;
    }
  }
  ...
  async getWalletData(telegramUserId: number): Promise<WalletInfo | null> {
    try {
      ...
      if (new Date() > secureData.expiresAt) {
        console.log(
          `[WalletStorage] Wallet data expired for user ${telegramUserId}`
        );
      ...
      console.error("[WalletStorage] Failed to retrieve wallet data:", error);
      return null;
    }
  }
  ...
  async clearWalletData(telegramUserId: number): Promise<void> {
    try {
      await this.deleteFromDatabase(telegramUserId);
      console.log(
        `[WalletStorage] Cleared wallet data for user ${telegramUserId}`
      );
    } catch (error) {
      console.error("[WalletStorage] Failed to clear wallet data:", error);
    }
  }
  ...
  private async saveToDatabase(data: SecureWalletData): Promise<void> {
    try {
      ...
      console.error("[WalletStorage] Database save error:", error);
      throw error;
    }
  }
  ...
  private async getFromDatabase(
    ...
    try {
      ...
      console.error("[WalletStorage] Database retrieval error:", error);
      return null;
    }
  }
  ...
  private async deleteFromDatabase(telegramUserId: number): Promise<void> {
    try {
      ...
      console.error("[WalletStorage] Database deletion error:", error);
      throw error;
    }
  }
}
  ...
  async checkWallet(
    ...
    try {
      ...
      if (!forceRefresh) {
        ...
        if (cachedWallet) {
          console.log(
            `[WalletAPI] Using cached wallet data for user ${telegramUserId}`
          );
          return {
            has_wallet: true,
      ...
      console.log(
        "[WalletAPI] Fetching wallet data from API for user",
        telegramUserId
      );
      ...
      if (walletResponse.has_wallet && walletResponse.wallet_info) {
        try {
          ...
          console.log(
            `[WalletAPI] Stored wallet data for user ${telegramUserId}`
          );
        } catch (storageError) {
          console.warn(
            "[WalletAPI] Failed to store wallet data:",
            storageError
          );
...
export async function getWalletInfo(
  ...
  try {
    ...
    if (forceRefresh) {
      ...
      if (!isHealthy) {
        console.warn("SolviumAI API is not healthy");
        return null;
      }
    }
