...
prepare_config_dir() {
    local config_dir="$HOME/Documents/.cursor-free-vip"
    echo -e "${CYAN}ℹ️ Preparing config directory...${NC}"
    ...
    # Fix ownership issues if files are owned by root
    if [ -f "$config_dir/config.ini" ] && [ "$(stat -c %U "$config_dir/config.ini" 2>/dev/null)" = "root" ]; then
        echo -e "${YELLOW}⚠️ Fixing ownership of config files...${NC}"
        sudo chown -R "$USER:$USER" "$config_dir"
        echo -e "${GREEN}✅ Fixed ownership of config directory${NC}"
    fi
...
install_cursor_free_vip() {
    ...
    if [ -f "${binary_path}" ]; then
        ...
        # Prepare config directory first
        prepare_config_dir
        
        # Try to run without sudo first
        echo -e "${CYAN}ℹ️ Attempting to run without elevated privileges...${NC}"
        chmod +x "${binary_path}"
        ...
        if "${binary_path}"; then
            ...
            echo -e "${YELLOW}⚠️ Failed to run normally, trying with sudo...${NC}"
            if command -v sudo >/dev/null 2>&1; then
                echo -e "${CYAN}ℹ️ Starting program with sudo...${NC}"
                sudo "${binary_path}"
            else
                echo -e "${RED}❌ sudo not found and normal execution failed${NC}"
                exit 1
            fi
        fi
        return
