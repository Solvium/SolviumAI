...
async def get_leaderboard(
    leaderboard_type: str = Query("total", description="Type of leaderboard: total, creator, or taker"),
    ...
    try:
        ...
        leaderboard = await PointService.get_leaderboard(limit=limit, leaderboard_type=leaderboard_type)
...
async def get_point_system_stats():
    ...
    try:
        ...
        total_leaderboard = await PointService.get_leaderboard(limit=10, leaderboard_type="total")
        creator_leaderboard = await PointService.get_leaderboard(limit=10, leaderboard_type="creator")
        taker_leaderboard = await PointService.get_leaderboard(limit=10, leaderboard_type="taker")
        
        # Calculate some basic stats
        total_users = len(total_leaderboard)
        total_points_distributed = sum(entry["total_points"] for entry in total_leaderboard)
...
async def get_user_rank(user_id: str):
    ...
    try:
        ...
        leaderboard = await PointService.get_leaderboard(limit=1000, leaderboard_type="total")
