...
async def show_reward_structure_options(update, context, reward_amount):
    ...
    progress_text += f"• Top 5 winners: {reward_amount} NEAR (40/25/15/12/8 distribution)\n"
...
async def show_distribution_preview(update, context, structure_type: str):
    ...
    buttons = [
        ...
        [InlineKeyboardButton(
            "🔙 Back to Options",
            callback_data="back_to_structure_options"
        )]
    ]
    reply_markup = InlineKeyboardMarkup(buttons)
...
async def confirm_distribution_structure(update, context):
    ...
    if choice == "confirm_structure_top_5":
        ...
        reward_amount = await redis_client.get_user_data_key(user_id, "reward_amount")
        await redis_client.set_user_data_key(user_id, "total_cost", float(reward_amount))
...
async def handle_quiz_interaction_callback(update, context):
    ...
    logger.info(
        f"DEBUG: handle_quiz_interaction_callback called - user_id: {user_id}, callback_data: {query.data}"
    )
    ...
    try:
        ...
        logger.info(
            f"DEBUG: Parsed callback data - action: {action}, quiz_id: {quiz_id}"
        )

        if action == "play_quiz":
            logger.info(f"DEBUG: Calling handle_play_quiz for action: {action}")
            await handle_play_quiz(update, context, quiz_id)
            logger.info(f"DEBUG: handle_play_quiz completed for action: {action}")
...
async def handle_play_quiz(update, context, quiz_id):
    ...
    logger.info(
        f"DEBUG: handle_play_quiz called - user_id: {user_id}, quiz_id: {quiz_id}, chat_id: {chat_id}"
    )
    ...
    try:
        ...
        try:
            ...
            logger.info(f"DEBUG: About to call start_quiz_for_user for user {user_id}")
            # Start quiz for user
            logger.info(
                f"DEBUG: About to call start_quiz_for_user from handle_play_quiz"
            )
            await start_quiz_for_user(update, context, quiz)
            logger.info(f"DEBUG: start_quiz_for_user completed from handle_play_quiz")
...
async def start_quiz_for_user(update, context, quiz):
    ...
    logger.info(
        f"DEBUG: start_quiz_for_user called - user_id: {user_id} (type: {type(user_id)}), chat_id: {chat_id}"
    )
    ...
    try:
        ...
        logger.info(f"DEBUG: Wallet check result for user {user_id}: {has_wallet}")
        ...
        if not has_wallet:
            ...
            try:
                ...
                # Format the wallet info message
                logger.info(f"DEBUG: Formatting wallet info message")
                wallet_message, mini_app_keyboard = (
                    await wallet_service.format_wallet_info_message(wallet_info)
                )
                logger.info(
                    f"DEBUG: Wallet message formatted successfully, length: {len(wallet_message)}"
                )
        ...
        try:
            ...
            if reward_structure == "winner_takes_all" and reward_amount:
                ...
                if payment_method == "TOKEN" and token_contract_address:
                    try:
                        token_symbol = token_contract_address.split(".")[0].upper()
                        details_parts = [f"{amount} {token_symbol} for {place}" 
                                       for amount, place in zip(amounts, places)]
                    except Exception as e:
                        logger.error(f"Error getting token symbol: {e}")
                        details_parts = [f"{amount} TOKEN for {place}" 
                                       for amount, place in zip(amounts, places)]
                else:
                    details_parts = [f"{amount} NEAR for {place}" 
                                   for amount, place in zip(amounts, places)]
