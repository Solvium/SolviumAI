...
def run_migration():
    ...
    try:
        ...
        for user in existing_users:
            existing_points = session.query(UserPoints).filter(
        ...
        quizzes_without_creator = session.query(Quiz).filter(
            Quiz.creator_id.is_(None)
        ).all()
        ...
        logger.info(f"   - {len(quizzes_without_creator)} quizzes without creator_id (left as NULL)")
...
def rollback_migration():
    ...
    try:
        ...
        if table_exists('quizzes') and column_exists('quizzes', 'creator_id'):
            ...
            session.execute(text("ALTER TABLE quizzes DROP COLUMN IF EXISTS creator_id"))
