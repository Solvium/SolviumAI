...
class CacheService:
    ...
    async def get_cached_wallet(self, user_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if db_wallet:
                ...
                logger.info(
                    f"Retrieved wallet from database and cached for user {user_id}"
                )
                return db_wallet
    ...
    async def set_cached_wallet(
        self, user_id: int, wallet_data: Dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            ...
            await self.redis_client.set_value(
                cache_key, wallet_data, ttl_seconds=self.WALLET_CACHE_TTL
            )
    ...
    async def invalidate_wallet_cache(self, user_id: int) -> bool:
        """
        Invalidate all wallet-related cache when data changes
        """
        try:
            cache_keys = [
                f"wallet:{user_id}",
            ...
            # Also clear legacy Redis user data keys
    ...
    async def set_cached_balance(self, account_id: str, balance: str) -> bool:
        ...
        try:
            cache_key = f"balance:{account_id}"
            await self.redis_client.set_value(
                cache_key, balance, ttl_seconds=self.BALANCE_CACHE_TTL
            )
    ...
    async def set_cached_user(self, user_id: int, user_data: Dict[str, Any]) -> bool:
        ...
        try:
            cache_key = f"user:{user_id}"
            await self.redis_client.set_value(
                cache_key, user_data, ttl_seconds=self.USER_CACHE_TTL
            )
    ...
    async def cache_wallet_creation(
        self, user_id: int, wallet_info: Dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            ...
            await self.redis_client.set_value(
                f"wallet_created:{user_id}", "true", ttl_seconds=self.SESSION_CACHE_TTL
            )
    ...
    async def has_cached_wallet(self, user_id: int) -> bool:
        ...
        try:
            ...
            wallet_created = await self.redis_client.get_value(
                f"wallet_created:{user_id}"
            )
            ...
            wallet_created = await self.redis_client.get_user_data_key(
                user_id, "wallet_created"
            )
            if wallet_created == "true":
                return True
