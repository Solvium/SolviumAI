...
class PointService:
    ...
    async def award_quiz_taker_points(
        ...
        try:
            ...
            logger.info(f"Awarded {points_awarded} points to user {user_id} for quiz {quiz_id}")
    ...
    async def award_quiz_creator_points(
        ...
        try:
            ...
            logger.info(f"Awarded {points_awarded} points to creator {creator_user_id} for quiz {quiz_id}")
    ...
    @staticmethod
    async def get_leaderboard(limit: int = 50, leaderboard_type: str = "total") -> List[Dict]:
        """
        ...
        try:
            ...
            leaderboard_query = session.query(
                ...
                User.username,
                User.first_name
            ).join(User, UserPoints.user_id == User.id).order_by(
                desc(sort_column),
                desc(UserPoints.last_updated)
            ).limit(limit)
            
            leaderboard = []
            for rank, (user_points, username, first_name) in enumerate(leaderboard_query, 1):
                leaderboard.append({
                    "rank": rank,
                    "user_id": user_points.user_id,
    ...
    async def get_user_point_history(user_id: str, limit: int = 50) -> List[Dict]:
        ...
        session = SessionLocal()
        try:
            transactions = session.query(PointTransaction).filter(
    ...
    async def check_first_correct_answer(quiz_id: str, question_index: int) -> bool:
        ...
        try:
            ...
            existing_correct = session.query(QuizAnswer).filter(
                QuizAnswer.quiz_id == quiz_id,
