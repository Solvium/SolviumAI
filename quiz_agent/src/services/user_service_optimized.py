...
class OptimizedUserService:
                ...
                with SessionLocal() as session:
                    for user_data in user_data_list:
                        try:
                            ...
                            existing_user = await self.get_user_profile(
                            ...
                            logger.error(
                                f"Error creating user {user_data.get('telegram_id')}: {e}"
                            )
                            continue
    ...
    async def link_wallet_optimized(self, update: Update, context: CallbackContext):
        ...
        try:
            ...
            if update.effective_chat.type != "private":
                ...
                logger.info(
                    f"User {user_id_str} initiated wallet linking from group chat"
                )
            ...
            logger.error(
                f"Error in optimized wallet linking for user {user_id_str}: {e}"
            )
            await safe_send_message(
                context.bot,
                update.effective_chat.id,
    ...
    async def _process_wallet_address(
        self, update: Update, context: CallbackContext, user_id_str: str
    ):
        ...
        try:
            ...
            await RedisClient.set_value(
                f"wallet_link:{wallet_address}", user_id_str, ttl_seconds=3600
            )
            ...
            logger.info(
                f"User {user_id_str} successfully linked wallet {wallet_address}"
            )
    ...
    async def _initiate_wallet_linking(
        self, update: Update, context: CallbackContext, user_id_str: str
    ):
        ...
        try:
            ...
            await safe_send_message(
                context.bot, update.effective_chat.id, wallet_prompt
            )
    ...
    def _validate_wallet_address(self, address: str) -> bool:
        ...
        if Config.is_development():
            ...
            return (
                ...
                or address.endswith(".near")
                or len(address) == 64  # Implicit account
            )
        else:
            # Production - only mainnet addresses
            return address.endswith(".near") or len(address) == 64  # Implicit account
    ...
    async def cleanup_inactive_cache(self, days: int = 7):
        ...
        try:
            ...
            if inactive_keys:
                ...
                logger.info(
                    f"Cleaned up {len(inactive_keys)} inactive user cache entries"
                )
