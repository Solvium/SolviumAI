    ...
    async def get_performance_summary(self, hours: int = 1) -> Dict[str, Any]:
        ...
        try:
            ...
            summary = {
                ...
                "median_response_time_ms": sorted(response_times)[
    ...
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.debug(
                    f"Cache miss for {func.__name__} - executed in {execution_time:.2f}ms"
                )
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if response_time > 1000:  # > 1 second
                    logger.warning(
                        f"Slow operation detected: {endpoint} took {response_time:.2f}ms"
                    )
...
class BulkOperationManager:
    ...
    async def add_operation(self, operation_type: str, data: Any):
        ...
        should_flush = (
            len(self.pending_operations[operation_type]) >= self.batch_size
    ...
    async def flush_operations(self, operation_type: str = None):
        ...
        if operation_type:
            operations_to_flush = {
                operation_type: self.pending_operations.get(operation_type, [])
            }
        else:
...
async def optimize_quiz_cache(quiz_id: str, user_count: int = 0):
    ...
    try:
        ...
        await RedisClient.set_value(
            f"cache_settings:{quiz_id}", cache_settings, ttl_seconds=3600
        )

        logger.debug(
            f"Optimized cache settings for quiz {quiz_id} with {user_count} users"
        )
